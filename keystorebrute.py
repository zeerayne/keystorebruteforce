...
def getoptions():
    ...
    parser.add_option(
        ...
        dest="keystorefile",
        default=None,
        help="A file with a list of keystores to crack",
    )
    parser.add_option(
        "-P",
        "--print-dumps",
        ...
        dest="printdumps",
        default=False,
        help="If specified, the keystore information will be dumped in the terminal",
    )
    parser.add_option(
        "-S",
        "--save-dumps",
        ...
        dest="savedumps",
        default=False,
        help="Save dumps of the data to the output folder [default: False]",
    )
    parser.add_option(
        "-d", "--dictionary", default="dictionary.txt", dest="dictionary", help="Dictionary to use in the brute force"
    )
    parser.add_option(
        "-v",
        "--verbose",
        ...
        dest="verbose",
        default=False,
        help="Show password being tested [default: False]",
    )
...
def print_pem(data, data_type):
    ...
    print("\r\n".join(textwrap.wrap(base64.b64encode(data).decode("utf-8"), 64)))
...
def dumpInfo(ksname, data, data_type):
    ...
    if options.savedumps:
        ...
        if not os.path.exists(CURRENTOUT):
            os.mkdir(CURRENTOUT)
        savepath = "%s/%s" % (CURRENTOUT, re.sub(r"[^a-zA-Z\.\-]", "_", ksname))
...
def dictionaryAttack(kspath, dictionary):
    ...
    for passw in dictines:
        ...
        if foundpwd:
            break
    if foundpwd is False and NotValidFile is False:
